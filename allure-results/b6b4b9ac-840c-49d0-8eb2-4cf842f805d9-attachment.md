...
- Name: Basics\SkipTest.spec.ts >> Login2
- Location: playwright_1\tests\Basics\SkipTest.spec.ts:27:1
...
```ts
  ...
  9  | 
  10 |     await expect.soft(page.getByRole("link",{name:"Home "})).toBeVisible()
  11 |     await expect(page.locator("#nameofuser")).toHaveText("Welcome admin")
  12 | 
  13 | })
  14 | 
  15 | test.skip("Login1", async({page})=>{
  16 |     await page.goto("https://www.demoblaze.com/index.html")
  17 |     await page.locator("#login2").click()
  18 |     await page.locator("#loginusername").fill("admin")
  19 |     await page.locator("#loginpassword").fill("admin")
  20 |     await page.getByRole("button",{name:"Log in"}).click()
  21 | 
  22 |     await expect.soft(page.getByRole("link",{name:"Home "})).toBeVisible()
  23 |     await expect(page.locator("#nameofuser")).toHaveText("Welcome admin")
  24 | 
  25 | })
  26 | 
  27 | test("Login2", async({page})=>{
  28 |     await page.goto("https://www.demoblaze.com/index.html")
  29 |     await page.locator("#login2").click()
  30 |     await page.locator("#loginusername").fill("admin")
  31 |     await page.locator("#loginpassword").fill("admin")
  32 |     await page.getByRole("button",{name:"Log in"}).click()
  33 | 
  34 |     await expect.soft(page.getByRole("link",{name:"Home "})).toBeVisible()
> 35 |     await expect(page.locator("#nameofuser").textContent()).toBe("Welcome admin")
     |                                                             ^ Error: expect(received).toBe(expected) // Object.is equality
  36 | 
  37 | })
```