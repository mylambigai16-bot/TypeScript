...
- Name: SearchTest.test.ts >> Searching Product @smoke >> search product @smoke
- Location: tests\SearchTest.test.ts:10:9
...
Locator: locator('//h4')
Expected: visible
Error: strict mode violation: locator('//h4') resolved to 3 elements:
    1) <h4>…</h4> aka getByRole('heading', { name: 'MacBook', exact: true })
    2) <h4>…</h4> aka getByRole('heading', { name: 'MacBook Air' })
    ...
            ...
            - generic [ref=e111]:
              - heading "MacBook" [level=4] [ref=e112]:
                - link "MacBook" [ref=e113] [cursor=pointer]:
                  - /url: https://tutorialsninja.com/demo/index.php?route=product/product&product_id=43&search=MacBook
              - paragraph [ref=e114]: Intel Core 2 Duo processor Powered by an Intel Core 2 Duo processor at speeds up to 2.16GHz, t..
              - paragraph [ref=e115]:
                - text: $602.00
...
```ts
  ...
  3  | 
  4  | test.describe("Searching Product @smoke", () => {
  5  | 
  6  |     test.beforeEach("Before Search @smoke", async ({ homePage }) => {
  7  |         await homePage.navigate()
  8  |     })
  9  | 
  10 |     test("search product @smoke", async ({ homePage, searchPage }) => {
  11 |         await homePage.searching("MacBook")
  12 |         await homePage.clcikSearchIcon()
  13 |         const products = await searchPage.listProduct();
  14 |         const listP = await products.allInnerTexts();
  15 |         for (var pro of listP)
  16 |            expect(pro).toContain("MacBook")
> 17 |         await expect(products).toBeVisible();
     |                                ^ Error: expect(locator).toBeVisible() failed
  18 |     })
  19 | 
  20 | })
